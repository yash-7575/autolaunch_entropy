...
const PostsIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
    <path d="M8 6H21M8 12H21M8 18H21M3 6H3.01M3 12H3.01M3 18H3.01" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const firstMenu: MenuItem[] = [
  { name: 'Calendar', icon: <CalendarIcon />, path: '/launches' },
  { name: 'Posts', icon: <PostsIcon />, path: '/posts' },
  { name: 'Agent', icon: <AgentIcon />, path: '/agents' },
  { name: 'Analytics', icon: <AnalyticsIcon />, path: '/analytics' },
  { name: 'Media', icon: <MediaIcon />, path: '/media' },
  { name: 'Plugs', icon: <PlugsIcon />, path: '/plugs' },
  { name: 'Integrations', icon: <IntegrationsIcon />, path: '/third-party' },
];
        ...
        <div className={styles.sidebarInner}>
          <div className={styles.sidebarContent}>
            ...
            <div className={styles.menuTop}>
              {firstMenu.map((item) => <SidebarItem key={item.name} item={item} />)}
            </div>
            <div className={styles.menuBottom}>
              {secondMenu.map((item) => <SidebarItem key={item.name} item={item} />)}
            </div>
          </div>
        </div>
      </div>
